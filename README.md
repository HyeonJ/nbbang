...
> **상태**: **v1 완성**(F1~F7)·배포 — https://nbbang-iota.vercel.app
...
E2E 스펙 5본이 각각 다른 주장을 맡는다: `foundation`(가입·개설·합류·권한),
`ledger`(잔액 = 원장 합산 + CSV 정확성·유출 금지), `authz`(쓰기 액션 7종 × 역할 5종),
`public-ledger`(링크 하나로 열리되 토큰·이메일·계좌는 새지 않음), `settlement`(엔빵 계산 결과와
**정산이 잔액·공개 장부·CSV를 하나도 움직이지 않음** — ADR-003을 세 표면에서 확인한다).
