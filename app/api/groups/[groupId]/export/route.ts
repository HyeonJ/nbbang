 ...
 * 골라 그것을 구조적으로 보장한다.
 *
 * 계좌 문구(`groups.accountLabel`)도 **CSV에 넣지 않는다** — 이것은 원장 내보내기이지
 * 모임 설정 내보내기가 아니다(schema.ts의 3줄 규칙 ③). 아래 `COLUMNS`에 그 열이 없고,
 * `e2e/ledger.spec.ts`의 금칙 목록이 계좌 문구를 파일 본문에서 매 푸시마다 다시 찾아본다.
 *
 * ⚠️ 뮤테이션으로 확인한 사실(Task 10): **열을 하나 늘리는 유출은 금칙 목록이 잡지 않는다** —
 * 그보다 앞에 있는 헤더 행 단언(`일자,종류,…,정정대상`)이 먼저 빨개져 거기서 끝난다.
 * 금칙 목록이 실제로 일하는 것은 **열 구조를 그대로 둔 채 셀 안에 흘리는** 유출이며
 * (메모에 계좌를 이어 붙여 재현), 그 형태에서 `CSV에 accountLabel이 들어 있다`로 빨개졌다.
 * 즉 이 파일의 방어는 두 겹이다: 열 정의(구조)와 금칙 목록(내용). 둘 다 필요하다.
 */
